...
class Ocr:
    ...
    def binarise_greyscale_image(self, im, threshold=150):
    ...
    def is_color_similar(self, color1, color2, similarity):
    ...
    def binarise_color_image(self, im, wantedcolor="white", similarity=None):
        # 6 seconds as well
        ...
        for i in range(im.size[0]):
            for j in range(im.size[1]):
                pixels[i, j] = (255, 255, 255) if (self.is_color_similar(pixels[i, j], wantedcolor, 30)) else (0, 0, 0)
        return im
    ...
    def ocr_using_greyscale(self, image, threshold=200):
        black_white_image = self.binarise_greyscale_image(image, threshold)
    ...
    def ocr_using_color_similarity(self, image, color="white"):
        black_white_image = self.binarise_color_image(image, color)
        return pytesseract.image_to_string(black_white_image).lower().replace("%", "")
