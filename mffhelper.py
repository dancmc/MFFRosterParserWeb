...
class Rects:
    def __init__(self, width: int, height: int):
        ...
        if aspect == 16 / 9:
            ...
            self.list_detail_rects = {
                "rect_tier": scale_rect((460, 393, 555, 427)),
                "rect_char": scale_rect((104, 479, 546, 520)),
                "rect_uni": scale_rect((108, 523, 554, 560)),
                "rect_phys_att": scale_rect((911, 240, 1186, 280)),
                "rect_energy_att": scale_rect((911, 280, 1186, 327)),
                "rect_atk_spd": scale_rect((911, 327, 1186, 374)),
                "rect_crit_rate": scale_rect((911, 374, 1186, 423)),
                "rect_crit_dam": scale_rect((911, 423, 1186, 468)),
                "rect_def_pen": scale_rect((911, 468, 1186, 517)),
                "rect_ignore_dodge": scale_rect((911, 517, 1186, 568)),
                "rect_phys_def": scale_rect((1530, 238, 1816, 280)),
                "rect_energy_def": scale_rect((1530, 280, 1816, 329)),
                "rect_hp": scale_rect((1530, 329, 1816, 376)),
                "rect_recorate": scale_rect((1530, 376, 1816, 421)),
                "rect_dodge": scale_rect((1530, 421, 1816, 472)),
                "rect_mv_spd": scale_rect((1541, 810, 1814, 847)),
                "rect_debuff": scale_rect((1653, 856, 1816, 895)),
                "rect_scd": scale_rect((1666, 903, 1816, 937))}
...
def get_char_json(filename):
    ...
    if greyscale_ocr(screenshot, rects.rect_check_details_page, threshold=100) == "attack":
        ...
        char.uniform_alias = get_uniform_alias(greyscale_ocr(screenshot, rects.rect_uni, 180))
        ...
        char.scd = greyscale_ocr(screenshot, rects.rect_phys_att, 240)

        # print(jsonpickle.encode(char, unpicklable=False))
        return jsonpickle.encode(char, unpicklable=False)
        ...
        # returns list of tuples from DB with format (char_alias, gear_name, gear_number)
        char_list = get_chars_from_gear(gear_name)

        # if managed to match to exactly 1 character, return character json, else :
        char = Character()
        if len(char_list) == 1:
            char.id = char_list[0][0]
            gear_num = char_list[0][2]
            ...
            return char_list, gear_result, filename
    else:
        return None
